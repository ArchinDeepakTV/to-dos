...
export default function RegisterPage() {
  ...
  const [password_again, set_password_again] = React.useState("");
  const [password, set_password] = React.useState("");
  const [styles, set_styles] = React.useState({ display: "none" });
  const [style_password, set_style_password] = React.useState({
    display: "none",
  });
  ...
  const handle_form = async () => {
    if (
      ...
      let new_entry = { mail_id: mail_id, password: password, name: name };
      await fetch(
        `${nodeBackendURL}/DB-create-user`,
        {
          method: "post",
  ...
  return (
    ...
    >
      ...
      <div>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (password === e.target.value) {
                set_style_password({ display: "none" });
                set_password_again(e.target.value);
              } else {
                set_style_password({
                  textAlign: "center",
        ...
        <div>
          <h4 style={styles}>
            Entered Mail ID or Password doesn't fulfil our requirements
          </h4>
          <h4 style={style_password}>Entered passwords don't match</h4>
        </div>
      </div>
    </div>
  );
